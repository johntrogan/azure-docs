...
2. **Recreates the collection** — Deletes the existing collection (if any) and creates a fresh one with `auto_tenant_creation=True`. Tenants are created on the fly when documents are inserted, so you don't need to pre-register department names.
