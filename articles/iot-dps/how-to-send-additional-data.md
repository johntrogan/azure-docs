...
When your device calls [Register Device](/rest/api/iot-dps/data-plane/runtime-registration/register-device
) to register with DPS, it can include more data in the **payload** property. For example, the following JSON shows the body for a request to register using TPM attestation:
...
). For example, the following JSON shows the body of a successful response to register using TPM attestation.
