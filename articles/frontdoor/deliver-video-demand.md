...
By routing traffic across Microsoft's private network between these distributed edge locations, Azure Front Door can minimize latency, avoid congested public internet paths, and provide highly resilient delivery infrastructure. Combined with Microsoft's extensive peering relationships and redundant network paths, this enables streaming workloads to reach deep into regional and last-mile networks and deliver a stable playback experience for end users with minimal buffering, whether your audience is a few thousand internal viewers or a global audience of concurrent users.
...
|---------------------------|---------------|----------------|
| Rules | Disable compression for segment files (*.m4s*, *.mp4*) | DASH and HLS media segments are already compressed video and audio data. Re-compressing them provides no benefit and introduces unnecessary CPU overhead and latency during delivery. |
| Rules | Enable compression for manifest and playlist files (*.mpd*, *.m3u8*) | DASH and HLS manifests are small text-based XML or plain-text files that compress very efficiently (often 70–80% size reduction). Enabling compression reduces manifest transfer time and improves initial player startup and playlist refresh performance. |
| Rules | Cache *.m3u8*, *.mpd* playlist files for 3 hours | For VOD, playlist files are static after publication. They do not change between viewer sessions. Caching them aggressively at the edge eliminates repeated round-trips to origin and accelerates startup time for every viewer. <br> **Note:** this TTL is specific to VOD and must not be applied to live streaming manifests. |
...
| Route | Query string caching behavior: *Ignore query string* | This reinforces the rule-level configuration and ensures consistent cache behavior for all requests matching the route, including requests that do not match a more specific rule. |
| Route | Forwarding protocol: **HTTP only** (to origin) | If the origin resides within a trusted private network boundary behind Azure Front Door — such as Azure Blob Storage on a private endpoint — removing the TLS negotiation hop between AFD edge and origin reduces latency on every cache miss. <br> **Caveat:** if your content is DRM-protected (e.g., PlayReady, Widevine) or your origin serves license or key material, maintain HTTPS to origin regardless of network topology. The sensitivity of encryption keys warrants the added latency overhead. |
| Route | Redirect client traffic to **HTTPS** | Viewer traffic remains encrypted end-to-end from the client to the AFD edge, which is the required security posture for any media delivery workload — and is essential for DRM playback environments where browsers enforce secure contexts. |
| Route | Pattern match /content/* (scope to streaming path) | Scoping rules to the specific content path ensures that these optimizations apply only to streaming assets and do not inadvertently affect other endpoints - such as APIs, auth services, or web app traffic - served by the same AFD instance. |
...
- **CORS headers:** Any video player running in a browser will issue cross-origin requests for media segments and manifest files. Without correct Cross-Origin Resource Sharing (CORS) headers — specifically Access-Control-Allow-Origin on segment and manifest responses — browser-based playback will fail entirely due to browser security policy enforcement. Configure CORS headers either at the origin or via AFD's response header modification rules. This is particularly important when your player UI is served from a different domain than your media CDN endpoint.

- **DRM integration:** Commercial VOD and live streaming deployments that require content protection will use Digital Rights Management - typically some combination of Microsoft PlayReady, Google Widevine, and Apple FairPlay depending on the target device ecosystem. DRM license server requests must not be cached and should be routed separately from media segments. AFD can forward license requests to a DRM license origin while caching media segments independently. Plan your AFD route configuration to distinguish between licensable media paths and license acquisition endpoints from the start.

- **Content protection:** Azure Front Door's WAF provides baseline geo-filtering and rate limiting at the policy level. Native cryptographic URL request signing at the edge is on the Azure Front Door roadmap but is not currently available in production releases. In the interim, access control for streaming content typically involves a lightweight origin-side or middleware token validation layer — for example, an Azure Function that issues short-lived access tokens which are validated before content requests are proxied through AFD. One important nuance for any signed URL approach in streaming: signed URLs applied to individual media segments fragment your CDN cache, since each viewer receives a unique signed URL and the CDN treats each variant as a distinct cache key. For high-scale streaming, the cache-friendly pattern is cookie-based session authentication - validate the viewer's entitlement once on manifest request, issue a short-lived session cookie, and let segment requests flow through cleanly with shared cache efficiency.

Additionally, Azure Front Door is releasing Edge Actions - serverless edge compute running directly on AFD's global edge network - Public Preview is Spring 2026, and GA will be in summer 2026 . Edge Actions will enable fully custom token and cookie-based authentication logic at the edge without origin round-trips: validate a viewer's JWT or entitlement token on the manifest request, issue a scoped session cookie, and enforce per-asset geo-restrictions in code rather than at the blunt WAF-policy level. DRM license request proxying - intercepting license requests at the edge, validating entitlements, and forwarding only authorized requests to the license origin - will also be expressible directly in edge logic. Until Edge Actions reaches GA, work with your Microsoft account team on the appropriate interim access control architecture for your content protection requirements.
...
The most significant near-term milestone is the release of Edge Actions - AFD's serverless edge compute runtime — arriving Spring 2026 as Public Preview. Edge Actions brings programmable logic to AFD's global edge, enabling custom authentication flows, per-request entitlement enforcement, dynamic manifest manipulation, and DRM license proxying without any origin round-trips. For streaming operators, this closes the last meaningful gap between AFD and dedicated media CDN platforms that have offered edge compute for years. Additional investments include expanded low-latency live streaming support, deeper integration with the Azure media ecosystem, and enhanced streaming-specific observability.
