...
- Subscriptions requiring elevated permissions appear in a labeled "Requires Owner or User Access Administrator role" section
- Your current role on each subscription is shown in the "User role" column
...
If you didn't have these roles on a subscription, it simply didn't appear. This created three problems:
...
**Subscriptions you can assign** — subscriptions where you have Owner or User Access Administrator permissions. You can select these to add them to your agent's monitoring scope.

**Requires Owner or User Access Administrator role** — subscriptions where you lack the permissions needed to grant the agent Reader access. These appear with disabled checkboxes and reduced opacity, showing they exist but can't be selected.
...
| Aspect | Before | After |
|--------|--------|-------|
| **Subscriptions without permissions** | Hidden — not visible in picker | Visible in "Requires Owner or User Access Administrator role" section |
...
2. Contact your Azure administrator or the subscription owner
3. Request the **Owner** or **User Access Administrator** role on the subscription
4. Return to the subscription picker — the subscription moves to the "Subscriptions you can assign" section
