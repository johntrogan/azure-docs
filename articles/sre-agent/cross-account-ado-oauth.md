...
> - A browser popup lets you authenticate—no service principals, PATs, or admin approval needed
...
2. **Select Azure DevOps** as the platform and choose **Auth** as the authentication method.
3. **Enter the organization name** of the target ADO org.
4. **Select "Sign in to Azure DevOps"**—a confirmation dialog shows the permissions being granted.
5. **Select "Authorize"**—a sign-in popup opens where you select or sign in with the account that has access to the target organization.
6. **Repositories from the other org appear** in the repository picker, sorted alphabetically.
