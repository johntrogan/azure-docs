...
In this quickstart, you set up your Microsoft Discovery environment to run your first AI-powered scientific investigation. You complete these tasks:
...
- **User Assigned Managed Identities (UAMI)** with the required Azure role assignments for your supercomputer, workspace, and Azure Blob Storage. See [Create a User Assigned Managed Identity (UAMI)](#1d-create-a-user-assigned-managed-identity-uami).
...
1. Select **Add**, then select **Add role assignment**.
   :::image type="content" source="media/quickstart/assign-role.jpg" alt-text="Screenshot showing the Add role assignment option in Access control (IAM)." lightbox="media/quickstart/assign-role.jpg":::
...
1. Select **+ Select members**, choose the members to assign this permission to, then select **Next**.
   :::image type="content" source="media/quickstart/assign-role-members.jpg" alt-text="Screenshot showing the Members tab for adding role assignment members." lightbox="media/quickstart/assign-role-members.jpg":::
...
1. For `workspaceSubnet` and `agentSubnet`, under **SubnetDelegation**, select `Microsoft.App/environments`.
   :::image type="content" source="media/quickstart/create-vnet-2.jpg" alt-text="Screenshot of the Create virtual network subnet page showing subnet delegation settings." lightbox="media/quickstart/create-vnet-2.jpg":::
1. Optionally, you can remove the `default` subnet from the list. 
1. Review and create the virtual network.
   :::image type="content" source="media/quickstart/create-vnet-1.jpg" alt-text="Screenshot of the Create virtual network page showing IP address configuration." lightbox="media/quickstart/create-vnet-1.jpg":::
...
:::image type="content" source="media/quickstart/assign-roles-uami-1.jpg" alt-text="Screenshot of the Azure portal showing UAMI role assignment." lightbox="media/quickstart/assign-roles-uami-1.jpg":::
...
- Ensure that the storage account has `Storage Blob Data Contributor` access to the UAMI created in the [previous step](#1d-create-a-user-assigned-managed-identity-uami).
...
1. Select **Add your client IP address** if you're accessing data over the internet, or ensure your client can access the storage account and VNet via private link, Site-to-Site VPN, or ExpressRoute.
   :::image type="content" source="media/quickstart/create-storage-blob-4.jpg" alt-text="Screenshot showing the networking configuration for the storage account." lightbox="media/quickstart/create-storage-blob-4.jpg":::
1. Select **Review + create**, then select **Create**.
...
1. Enter `discoveryoutputs` as the name and select **Create**.
   :::image type="content" source="media/quickstart/create-storage-blob-2.jpg" alt-text="Screenshot showing the Add container dialog with discoveryoutputs as the name." lightbox="media/quickstart/create-storage-blob-2.jpg":::
...
1. Select **Save**.
   :::image type="content" source="media/quickstart/create-storage-blob-3.jpg" alt-text="Screenshot showing the CORS configuration for the storage account blob service." lightbox="media/quickstart/create-storage-blob-3.jpg":::
...
1. Select **Create** and enter details such as Subscription ID, Resource Group name, Location, and Name, then select **Next**.
   :::image type="content" source="media/quickstart/create-supercomputer-1.jpg" alt-text="Screenshot showing the basic details page for creating a Microsoft Discovery Supercomputer." lightbox="media/quickstart/create-supercomputer-1.jpg":::
1. In the **Networking** tab, select the Virtual Network and `aksSubnet` created in [step 1c](#1c-create-a-virtual-network-and-subnets), then select **Next**.
   :::image type="content" source="media/quickstart/create-supercomputer-1-5.jpg" alt-text="Screenshot showing the networking configuration for the supercomputer." lightbox="media/quickstart/create-supercomputer-1-5.jpg":::
1. In the System SKU tab, select Standard_D4s_v6 as the System SKU for this deployment and select **Next**.
1. In the Identities tab, add the User Assigned Managed Identity (UAMI) created in [step 1d](#1d-create-a-user-assigned-managed-identity-uami) for the cluster identity, kubelet identity, and workload identity. Supercomputer instances use this managed identity to access data from your Azure resources. Once done, select **Next**.
   :::image type="content" source="media/quickstart/create-supercomputer-2.jpg" alt-text="Screenshot showing the identity configuration step for the supercomputer." lightbox="media/quickstart/create-supercomputer-2.jpg":::
   :::image type="content" source="media/quickstart/create-supercomputer-2-5.jpg" alt-text="Screenshot showing the UAMI assigned to the supercomputer." lightbox="media/quickstart/create-supercomputer-2-5.jpg":::
...
1. Once validation is successful, select **Create**.
   :::image type="content" source="media/quickstart/create-supercomputer-3.jpg" alt-text="Screenshot of the Microsoft Discovery Supercomputer overview page after creation." lightbox="media/quickstart/create-supercomputer-3.jpg":::
...
1. In the left pane, select **Nodepool** under **Settings**, then select **Create**.
   :::image type="content" source="media/quickstart/create-supercomputer-nodepool-1.jpg" alt-text="Screenshot showing the create nodepool option in the supercomputer settings." lightbox="media/quickstart/create-supercomputer-nodepool-1.jpg" :::
...
1. On the **Networking** tab, select the Virtual Network and `supercomputerNodepoolSubnet` created in [step 1c](#1c-create-a-virtual-network-and-subnets). This must be the same virtual network selected for the supercomputer in [step 2](#2-create-a-supercomputer), then select **Next**.
   :::image type="content" source="media/quickstart/create-supercomputer-nodepool-1-5.jpg" alt-text="Screenshot showing the create nodepool option in the supercomputer settings." lightbox="media/quickstart/create-supercomputer-nodepool-1-5.jpg":::
1. On the **VM configuration** tab, select the Virtual Machine SKU to use for the nodepool, then select **Next**. The selected SKU and quota must be available in the region where you deploy the nodepool.
   :::image type="content" source="media/quickstart/create-supercomputer-nodepool-2.jpg" alt-text="Screenshot showing the VM SKU selection for the nodepool." lightbox="media/quickstart/create-supercomputer-nodepool-2.jpg":::
1. In the **Scaling** section, enter the maximum node count that your nodepool can scale to, for example: 5 and select **Next**.
   :::image type="content" source="media/quickstart/create-supercomputer-nodepool-3.jpg" alt-text="Screenshot showing the scaling configuration for the nodepool." lightbox="media/quickstart/create-supercomputer-nodepool-3.jpg":::
...
1. Select **+ Create** and enter details such as Subscription, Resource Group, Name, and Region, then select **Next**.
   :::image type="content" source="media/quickstart/create-workspace-1.jpg" alt-text="Screenshot showing the basic details page for creating a Microsoft Discovery workspace." lightbox="media/quickstart/create-workspace-1.jpg":::
1. On the **Networking** tab, select "Public network access" as "Enable" for this exercise. After that, populate the details for Private Endpoint subnet, Agent subnet and Workspace subnet with the subnets created earlier in [Step 1c](#1c-create-a-virtual-network-and-subnets), then select **Next**.
   :::image type="content" source="media/quickstart/create-workspace-2.jpg" alt-text="Screenshot showing the Networking tab while creating a workspace." lightbox="media/quickstart/create-workspace-2.jpg":::
...
1. On the **Supercomputer** tab, select **Add Supercomputer** and select your subscription, resource group, and the supercomputer created in [step 2](#2-create-a-supercomputer), then select **Next**.
   :::image type="content" source="media/quickstart/create-workspace-3.jpg" alt-text="Screenshot showing the Supercomputer tab while creating a workspace." lightbox="media/quickstart/create-workspace-3.jpg"::: 
1. On the **Workspace Identity** tab, select **Add** under **User Assigned Managed Identity (UAMI)** and select the identity created in [step 1d](#1d-create-a-user-assigned-managed-identity-uami) to provide access to the workspace.
   :::image type="content" source="media/quickstart/create-workspace-4.jpg" alt-text="Screenshot showing the Workspace Identity tab with the UAMI added." lightbox="media/quickstart/create-workspace-4.jpg":::
1. Add tags as needed, and move to the next tab.
1. Review the Terms and Conditions, then select **Review + Create**.
1. Once validation is successful, select **Create**.
   :::image type="content" source="media/quickstart/create-workspace-5.jpg" alt-text="Screenshot of the Microsoft Discovery Workspace overview page after creation." lightbox="media/quickstart/create-workspace-5.jpg":::
...
1. Then select **Review + create** button at the bottom and select **Create**.
   ...
   :::image type="content" source="media/quickstart/create-chat-model-1.jpg" alt-text="Screenshot of the Chat Model Deployment creation page." lightbox="media/quickstart/create-chat-model-1.jpg":::
...
1. Select the storage account created in [step 1e](#1e-create-an-azure-blob-storage-account).
   :::image type="content" source="media/quickstart/create-storage-containers-1.jpg" alt-text="Screenshot showing the Storage Container creation page in Microsoft Discovery Studio." lightbox="media/quickstart/create-storage-containers-1.jpg":::
1. Select **Create**.
...
1. Select **Create**.
   :::image type="content" source="media/quickstart/create-project-1.jpg" alt-text="Screenshot showing the Project creation page in Microsoft Discovery Studio." lightbox="media/quickstart/create-project-1.jpg":::
...
- [Get started with Microsoft Discovery Project](quickstart-studio.md) to create an agent and an investigation to start chatting.