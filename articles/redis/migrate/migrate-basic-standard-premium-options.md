...
- **Full control.** You decide exactly when to cut over, and you can test the new instance before switching production traffic. In case you have multiple applications connecting to a shared Redis instance, you can choose to migrate one application at a time.
...
- **No control over when cutover happens.** You can use the tool to initiate migration, but no control on when the traffic cutover exactly happens during the migration.
...
- **Data sync not supported.** This tooling will orchestrate hostname/endpoint migration but does not migrate over any data. 
