...
Back up the VM to a different subscription | Supported when the VM and Recovery Services vault are in the same tenant and same region.<br/><br/>- Cross-subscription backup isn't supported for Confidential VMs (CVMs), including ADE encrypted CVMs.<br> - Moving backed-up data between vaults isn't supported. <br/>- Moving a Recovery Services vault across subscriptions or between resource groups also isn't supported when the vault contains cross-subscription protected Azure VM backup items. <br><br> See [Back up Azure VMs in a Recovery Services vault](backup-azure-arm-vms-prepare.md) for steps involved.
