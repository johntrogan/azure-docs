...
Back up the VM to a different region or subscription |Backing up a VM to a different region isn't supported.<br><br>Backing up a VM to a Recovery Services vault in a different subscription is supported when the VM and vault are in the same tenant. See [Back up Azure VMs in a Recovery Services vault](backup-azure-arm-vms-prepare.md).
