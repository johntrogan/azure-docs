---
title: Deploy SAP dialog instances with SAP ASCS/SCS high-availability VMs on RHEL
description: Learn how to deploy SAP PAS and AAS dialog instances on ASCS/SCS high-availability cluster VMs running Red Hat Enterprise Linux (RHEL) in Azure.
author: dennispadia
...
* [SAPonLinuxNotes](https://help.sap.com/docs/SUPPORT_CONTENT/home/3361892261.html) has the SAP Notes required for Linux.
...
This configuration is an addition to an already configured SAP ASCS/SCS cluster setup. PAS and AAS are installed on virtual host names, and each instance directory (`/usr/sap/<SID>/D<nr>`) is managed as a cluster resource. You place the instance directory file system on the same highly available storage that you used for ASCS and ERS instance configuration.

The following example describes the deployment and uses this system information:
...
* You can deploy only two dialog instances, PAS and one AAS, with an SAP ASCS/SCS cluster setup.
* If you want to scale out your SAP system with more application servers (like **sapa03** and **sapa04**), you can install them in separate VMs. Because PAS and AAS are installed on virtual host names, you can install more application servers by using either a physical or virtual host name in separate VMs. To learn how to assign a virtual host name to a VM, see [Use SAP Virtual Host Names with Linux in Azure](https://techcommunity.microsoft.com/t5/running-sap-applications-on-the/use-sap-virtual-host-names-with-linux-in-azure/ba-p/3251593).
...
* Consider sizing your VM SKUs appropriately based on the sizing guidelines. When another VM in the cluster is unavailable, you must factor in the cluster behavior where multiple SAP instances (ASCS, ERS, PAS, and AAS) might run on a single VM.
* You must install the dialog instances (PAS and AAS) running with an SAP ASCS/SCS cluster setup by using a virtual host name.
* You must also use the same storage solution of the SAP ASCS/SCS cluster setup to deploy PAS and AAS instances. For example, if you configured an SAP ASCS/SCS cluster by using NFS on Azure Files, you must use the same storage solution to deploy PAS and AAS.
* The instance directory `/usr/sap/<SID>/D<nr>` of PAS and AAS must be mounted on an NFS file system and is managed as a resource by the cluster.
...
* In a traditional SAP ASCS/SCS high-availability configuration, application server instances running on separate VMs aren't affected when an issue affects SAP ASCS and ERS cluster nodes. But with the cost-optimization configuration, either the PAS or AAS instance restarts when an issue affects one of the cluster nodes.
...
The configuration described in this article is an addition to your already configured SAP ASCS/SCS cluster setup. In this configuration, PAS and AAS are installed on virtual host names. The cluster manages the instance directories. Based on your storage, use the steps described in the following articles to configure the `SAPInstance` resource for the SAP ASCS and SAP ERS instance in the cluster.
...
After you install the **ASCS**, **ERS**, and **Database** instances by using Software Provisioning Manager (SWPM), follow the next steps to install the PAS and AAS instances.
...
1. Open the internal load balancer created for the SAP ASCS/SCS cluster setup.
...
1. **Backend Pool**: This pool remains the same because you deploy PAS and AAS on the same back-end pool.
...
The health probe configuration property `numberOfProbes`, otherwise known as **Unhealthy threshold** in the Azure portal, isn't respected. To control the number of successful or failed consecutive probes, set the property `probeThreshold` to `2`. It's currently not possible to set this property by using the Azure portal. Use either the [Azure CLI](/cli/azure/network/lb/probe) or the [Azure PowerShell](/powershell/module/az.network/new-azloadbalancerprobeconfig) command.

When you place VMs without public IP addresses in the back-end pool of an internal load balancer, there's no outbound internet connectivity. To allow routing to public end points, you need to perform extra configuration. For steps on how to achieve outbound connectivity, see [Public endpoint connectivity for virtual machines using Azure Standard Load Balancer in SAP high-availability scenarios](high-availability-guide-standard-load-balancer-outbound-connections.md).
...
Before you install the dialog instances, prepare both cluster nodes with host name resolution, shared directories, swap space, and firewall rules.

When steps in this article are marked with the following prefixes:
...
1. **[1]** Change the ownership of the `/usr/sap/SID/D02` folder after you mount the file system.
...
1. **[1]** Install the SAP NetWeaver PAS.
   ...
   Install the SAP NetWeaver PAS as root on the first node. Use a virtual host name that maps to the IP address of the load balancer front-end configuration for the PAS. For example, use `sappas`, `10.90.90.30`, and the instance number that you used for the probe of the load balancer, for example **02**.
...
1. **[1]** Update the `/usr/sap/sapservices` file.
...
1. **[1]** Configure a constraint to start the PAS resource group only after the ASCS instance is started.
...
1. **[2]** Change the ownership of the `/usr/sap/SID/D03` folder after you mount the file system.
...
1. **[2]** Install an SAP NetWeaver AAS.
   ...
   Install the SAP NetWeaver AAS as root on the second node. Use a virtual host name that maps to the IP address of the load balancer front-end configuration for the AAS. For example, use `sapaas`, `10.90.90.31`, and the instance number that you used for the probe of the load balancer, for example, **03**.
...
1. **[2]** Update the `/usr/sap/sapservices` file.
...
1. **[2]** Configure a constraint to start the AAS resource group only after the ASCS instance is started.

   ```bash
   sudo pcs constraint order g-NW1_ASCS then g-NW1_AAS kind=Optional symmetrical=false
   ```

## Configure post-installation settings for PAS and AAS

After you install PAS and AAS, complete these steps to enable the instances to run on any cluster node.

1. **[1]** For PAS and AAS to run on any cluster node (sap-cl1 or sap-cl2), you must copy the content in `$HOME/.hdb` of `<sid>adm` from both cluster nodes.
...
1. **[A]** Check the status of the cluster.
...
- [Azure Virtual Machines high availability for SAP NetWeaver on RHEL](high-availability-guide-rhel.md)
- [Azure Virtual Machines high availability for SAP NetWeaver on RHEL with NFS on Azure Files](high-availability-guide-rhel-nfs-azure-files.md)
- [Azure Virtual Machines high availability for SAP NetWeaver on RHEL with Azure NetApp Files](high-availability-guide-rhel-netapp-files.md)
