...
> [!IMPORTANT]
> Azure VMs using private link do not require the regional GAS endpoints to be whitelisted or publicly resolvable. Private link traffic is routed through the host connection rather than the internet, so the regional endpoint is never consulted in that path.
