...
2. When the Functions host starts in your local project folder, it writes information about your functions to the terminal output.
    ...
    This sample includes an Event Hubs trigger function that processes messages from an input Event Hub, adds metadata, and sends the processed messages to an output Event Hub.

3. To test the function, you can send messages to the input Event Hub using Azure CLI:

    ```azurecli
    az eventhubs eventhub send-messages \
    ...
    ```
    🔄 Event hub function processing 1 message(s)
    📨 Processing event: { id: '1', message: 'Hello from CLI' }
    ✨ Message processed: { id: '1', message: 'Hello from CLI', timestamp: '2024-11-10T10:30:15.123Z' }
    📤 Sent 1 message(s) to output Event Hub
    ```

4. When you're done, press Ctrl+C in the terminal window to stop the host process.
::: zone-end 
::: zone pivot="programming-language-python"
...
::: zone-end
::: zone pivot="programming-language-typescript"
3. Send test messages to the input Event Hub and verify that the function processes them and sends them to the output Event Hub.
::: zone-end
::: zone pivot="programming-language-csharp,programming-language-python,programming-language-typescript" 
## Redeploy your code
