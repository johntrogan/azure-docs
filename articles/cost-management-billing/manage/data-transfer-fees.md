...
* The at-cost data transfer only applies to Internet egress (routed via Routing preference transit ISP network) if the service is supported on the ISP path. The at-cost data transfer doesn’t apply to services with a routing preference using the MGN, unless that service isn’t supported on the ISP network. For customers with billing address in the UK, at-cost data transfer is available via the MGN when transferring data from UK datacenters.
